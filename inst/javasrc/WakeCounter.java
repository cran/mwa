...
class WakeCounter {
	...
	public static String[][] wakeCounting(String[] timevarinput, String[] spatvarinput, String[][] data, String[][] wakeindexinput, String[] wakeparams, String[] treatment, String[] control, String[] depvar, String[] matchColumns, String[] path){
		...
		double[][] distances = new double[wakeevents][events];
		for (int i=0;i<wakeevents;i++){
			for (int j=0;j<events;j++){
				try{
					distances[i][j] = getGreatCircleDistance(Double.parseDouble(data[wakeindex[i]][latlongindex[0]]),Double.parseDouble(data[wakeindex[i]][latlongindex[1]]), Double.parseDouble(data[j][latlongindex[0]]),Double.parseDouble(data[j][latlongindex[1]]));
				}
				catch ( NumberFormatException e){
					e.printStackTrace();
				}
			...
			for (int timewindow=0; timewindow<timevar.length; timewindow++){
				for (int event=0; event<wakeevents; event++){
					...
					while(check){
						...
						if (eventindex > -1){
							try{
								date1 = format.parse(data[eventindex][timeindex]);
								date2 = format.parse(data[wakeindex[event]][timeindex]);
							}
							catch ( ParseException e){
								e.printStackTrace();
							}
							datediff = (int) Math.floor((date2.getTime() - date1.getTime())/timescaling);
							spatialdiff = distances[event][eventindex];
					...
					while (check && !remove){
						...
						if (eventindex<events){
							...
							datediff = (int) Math.floor((date1.getTime() - date2.getTime())/timescaling);
							spatialdiff = distances[event][eventindex];
	 ...
	 * 
	 * @Author: Sebastian Schutte, ETH Zurich 2014
	 * 
	 * @param latPos
	 * @param longPos
	 * @param latTarget
